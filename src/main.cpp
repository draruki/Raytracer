...
void init(Context &ctx)
{
    ...
    rt::setupScene(ctx.rtx, (modelDir() + "gargo_lowpoly.obj").c_str());
...
void drawImage(Context &ctx)
{
	...
	glUniform1i(glGetUniformLocation(ctx.program, "gamma_correction_enable"), ctx.rtx.gamma_correction);
...
void showGui(Context &ctx)
{
	...
	if (ImGui::SliderInt("Jitter", &ctx.rtx.per_sample_jitter, 1, 100))
	{
		rt::resetAccumulation(ctx.rtx);
	}
	if (ImGui::SliderFloat("Refraction", &ctx.rtx.refractive_index, 0.1f, 5.f))
	{
		rt::resetAccumulation(ctx.rtx);
	}
	if (ImGui::ColorEdit3("Albedo", &ctx.rtx.albedo[0]))
	{
		rt::resetAccumulation(ctx.rtx);
	}
